...
@Component({
  selector: 'app-print-qc',
  templateUrl: './print-qc.component.html',
  styleUrls: ['./print-qc.component.scss']
})
export class PrintQcComponent implements OnInit, AfterViewChecked {
  ...
  ngOnInit(): void {
    ...
    this.qcService.getQc(id).subscribe(
      (s: any) => {
        this.invoice = s;
    ...
    this.qcService.getQcGroup(id).subscribe(
      (s: any) => {
        this.invoiceGroup = s;
      },
      (e) => {
  ...
  alignInvoice() {
    if (this.showInvoice) {
      ...
      lastProdRow.style.height = (650 - rowH) + 'px';
